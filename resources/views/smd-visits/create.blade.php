<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="mt-2 text-sm text-slate-500">Pilih outlet existing atau buat outlet baru, lalu isi aktivitas lapangan yang dilakukan.</p>
        </div>
    </x-slot>
        ...
        <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('smd-visits.store') }}" enctype="multipart/form-data" class="space-y-6" x-data="smdVisitForm()">
                ...
                <section class="rounded-[1.75rem] border border-white/80 bg-white/90 p-5 shadow-sm shadow-slate-200/60">
                    ...
                    <div class="mt-5 rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4" x-show="! creatingNewOutlet">
                        ...
                        <div x-show="selectedOutlet" class="mt-4 rounded-2xl border border-brand-100 bg-brand-50 px-4 py-4 text-sm text-slate-600">
                            ...
                            <p class="mt-3 text-xs font-semibold uppercase tracking-[0.14em] text-brand-700" x-text="selectedOutlet?.official_kode || 'Belum ada official kode'"></p>
                        </div>
                    ...
                    <div class="rounded-[1.75rem] border border-white/80 bg-white/90 p-5 shadow-sm shadow-slate-200/60">
                        ...
                        <div class="mt-4 space-y-3">
                            @foreach (['ambil_po' => 'Ambil PO', 'merapikan_display' => 'Merapikan Display', 'tukar_faktur' => 'Tukar Faktur', 'ambil_tagihan' => 'Ambil Tagihan'] as $value => $label)
                                <label class="flex items-start gap-3 rounded-[1.5rem] border border-slate-200 bg-slate-50 px-4 py-4 transition hover:border-slate-300">
                                    <input type="checkbox" name="activities[]" value="{{ $value }}" x-model="activities" class="mt-1 h-4 w-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" @checked(collect(old('activities', []))->contains($value))>
                                    <span>
                ...
                <div class="sticky bottom-4 z-20 flex justify-end">
                    <x-primary-button class="w-full justify-center sm:w-auto">Simpan Kunjungan SMD</x-primary-button>
                </div>
            </form>
        </div>
    </div>
