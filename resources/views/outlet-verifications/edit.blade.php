    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h2 class="mt-2 text-3xl font-semibold leading-tight text-slate-900">Verifikasi Outlet</h2>
                <p class="mt-2 text-sm text-slate-500">{{ $outlet->name }} · {{ $outlet->branch?->name }}</p>
            </div>
            <a href="{{ route('outlet-verifications.index') }}" class="app-action-secondary px-4 py-2">Kembali ke daftar</a>
        </div>
    </x-slot>
            ...
            <div class="grid gap-6 xl:grid-cols-[1fr_0.92fr]">
                <section class="app-panel overflow-hidden p-0 sm:p-0">
                    <div class="bg-slate-900 px-5 py-5 text-white sm:px-6">
                        <p class="text-xs font-semibold uppercase tracking-[0.22em] text-sky-300">Verification Panel</p>
                        <h3 class="mt-2 text-xl font-semibold">Aktifkan outlet pending</h3>
                        <p class="mt-2 max-w-2xl text-sm leading-7 text-slate-300">Isi official kode untuk mengaktifkan outlet. Data customer di halaman ini hanya ditampilkan sebagai referensi.</p>
                    </div>
                    ...
                    <form method="POST" action="{{ route('outlet-verifications.update', $outlet) }}" class="space-y-6">
                        ...
                        <div class="app-soft-panel p-5">
                            <div class="grid gap-5 sm:grid-cols-2">
                                <div>
                                    ...
                                    <div class="mt-2 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">{{ $outlet->name }}</div>
                                </div>
                                <div>
                                    <x-input-label value="Cabang" />
                                    <div class="mt-2 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">{{ $outlet->branch?->name }}</div>
                                </div>
                                <div>
                                    <x-input-label value="Kecamatan" />
                                    <div class="mt-2 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">{{ $outlet->district }}</div>
                                </div>
                                <div>
                                    <x-input-label value="Kota" />
                                    <div class="mt-2 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">{{ $outlet->city }}</div>
                                </div>
                        ...
                        <div class="grid gap-5 md:grid-cols-2">
                            <div>
                                ...
                                <div class="mt-2 rounded-lg border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700">{{ ucfirst($outlet->category) }}</div>
                            </div>

                            <div>
                                <x-input-label value="Status outlet" />
                                <div class="mt-2 rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm font-semibold text-amber-700">{{ $outlet->statusLabel() }}</div>
                            </div>
                ...
                <section class="space-y-6">
                    ...
                    <div class="app-panel p-5">
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            @empty
                                <div class="rounded-lg border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-center text-sm text-slate-500">Belum ada riwayat kunjungan untuk outlet ini.</div>
                            @endforelse
                        </div>
                    </div>
                </section>
